			...
			if(holder)
				if(!holder.fakekey || C.holder)
					if(check_rights_for(src, R_ADMIN))
						C << "<span class='adminooc'>[config.allow_admin_ooccolor && prefs.ooccolor ? "<font color=[prefs.ooccolor]>" :"" ]<span class='prefix'>OOC:</span> <strong>[keyname][holder.fakekey ? "/([holder.fakekey])" : ""]:</strong> <span class='message'>[msg]</span></span></font>"
					else
...
/client/verb/looc(msg as text)
	...
	if(!holder)
		if(mob.stat == DEAD)
			usr << "<span class='danger'>You are not alive enough to use LOOC.</span>"
			return
		if(prefs.muted & MUTE_OOC)
			src << "<span class='danger'>You cannot use OOC (muted).</span>"
			return
